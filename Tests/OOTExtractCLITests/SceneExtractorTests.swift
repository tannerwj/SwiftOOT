...
final class SceneExtractorTests: XCTestCase {
    func testExtractWritesSpot04SceneMetadataFromFixtureSource() throws {
        ...
        XCTAssertEqual(
            kokiriSpawns.prefix(5).map(\.position),
            [
    ...
    func testExtractIgnoresPreprocessorDirectivesInsideSceneCommandArrays() throws {
        let source = try makeFixtureSource(
            sceneSource: sceneSourceFixture.replacingOccurrences(
                of: "    SCENE_CMD_PATH_LIST(spot04_scenePathList_00030C),",
                with: """
                    #if 1
            ...
            roomSource: roomSourceFixture.replacingOccurrences(
                of: "    SCENE_CMD_ACTOR_LIST(78, spot04_room_0ActorEntry_000094),",
                with: """
                    #if 1
    ...
    func testExtractReadsSceneAssetsFromExtractedVersionDirectory() throws {
        let source = try makeFixtureSource(assetRootComponents: ["extracted", "ntsc-1.0", "assets", "scenes"])
        let output = try makeTemporaryDirectory()
        defer {
            try? FileManager.default.removeItem(at: source)
            try? FileManager.default.removeItem(at: output)
    ...
    func testExtractIgnoresNonAssetSceneLikeSources() throws {
        ...
        try write(
            ...
            """,
            to: source.appendingPathComponent("src/code/z_scene.c")
        )
        try write(entranceTableFixture, to: source.appendingPathComponent("include/tables/entrance_table.h"))
        try seedActorTableManifest(at: output)
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: scenesRoot.path))
    }
    ...
    private func makeFixtureSource(
        ...
        let sceneDirectory = assetRootComponents
            ...
            .appendingPathComponent("overworld")
            .appendingPathComponent("spot04")
    ...
    private func seedActorTableManifest(at output: URL) throws {
        let actorNames = [
...
private let sceneSourceFixture = """
